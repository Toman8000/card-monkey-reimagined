...
namespace CardMonkeyReimagined.Displays
{
    ...
    /// </summary>
    public class CardMonkeyReimaginedMultiDisplay : ModTowerDisplay<CardMonkeyReimagined>
    {
        private readonly int[] t = null!; // The tiers used for a particular instance of CardMonkeyReimaginedBaseDisplay

        /// <summary>
        /// All classes that derive from ModContent MUST have a zero argument constructor to work
        /// </summary>
        public CardMonkeyReimaginedMultiDisplay()
        {
        }

        public CardMonkeyReimaginedMultiDisplay(int[] tiers)
        {
            t = tiers;
        }
        ...
        public override string Name => nameof(CardMonkeyReimaginedMultiDisplay) + "-" + t.Printed();
            ...
            for (var i = 0; i <= 2; i++)
            {
                for (var j = 0; j <= 2; j++)
                {
                    for (var k = 0; k <= 2; k++)
                    {
                        ...
                        if (tiers.IsValid())
                        {
                            yield return new CardMonkeyReimaginedMultiDisplay(tiers);
                        }
                    }
                }
            }
...
# if DEBUG
            ...
            SetMeshTexture(node, nameof(CardMonkeyReimaginedBaseDisplay));
